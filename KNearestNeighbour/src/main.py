...
trainingData = [ [ Y[t], [ leftRight(X[t]), upDown(X[t]), upLeft_DownRight(X[t]), upRight_DownLeft(X[t]) ] ] for t in range(len(X)) ]
...
def testAcuracy(neighbours, testSize, distanceFunction):
	...
	testData = [ [ testY[t], [ leftRight(testImages[t]), upDown(testImages[t]), upLeft_DownRight(testImages[t]), upRight_DownLeft(testImages[t]) ] ] for t in range(testSize) ]
